...
def login():
    ...
    def dialog1():
        ...
        dialog1.response = res.text
        if res.status_code == 200:
            window.destroy()
        elif "Unable to log in with provided credentials." in res.text:
            box.showinfo('info', 'Invalid Login credentials')
            entry2.delete(0, END)
            entry2.insert(0, "")
        else:
            box.showinfo('Velocity Error',str(res.status_code))
            # pass
    ...
    window.mainloop()
    try:
        data = json.loads(dialog1.response)
